...
fit_once <- function(beta0,
  ...
  fit_clom_X7_X8   <- clom(c("X7", "X8"), rep(0, 6))
  fit_clom_X5678 <- clom(c("X5", "X6", "X7", "X8"), rep(0, 8))
  
  mslom <- function(covs = NULL) {
    dat_w <- if (is.null(covs)) {
      compute_ipw(sim_dat, use = use)
    } else {
      compute_ipw(sim_dat, covars = covs, use = use)
    }
    fit_mslom_optim(dat_w)
  }
  fit_msl_X7_X8   <- mslom(c("X7", "X8"))
  fit_msl_X1278 <- mslom(c("X1", "X2", "X7", "X8"))
  fit_msl_X3478 <- mslom(c("X3", "X4", "X7", "X8"))
  fit_msl_X5678 <- mslom(c("X5", "X6", "X7", "X8"))
  fit_msl_X123478 <- mslom(c("X1", "X2", "X3", "X4", "X7", "X8"))
  fit_msl_all <- mslom(paste0("X", 1:8))
  
  dat_w_full <- compute_ipw(sim_dat, covars = c("X7", "X8"),
                                  use = use)     # all six X’s
  ...
  models <- list(fit_nc,
                 fit_clom_X5_X6, fit_clom_X7_X8, fit_clom_X5678,
                 fit_msl_X7_X8, fit_msl_X1278, fit_msl_X3478, 
                 fit_msl_X5678, fit_msl_X123478, fit_msl_all,
                 fit_wclom_all_cov)
  
  return(list(summary_table = tibble(
    Model   = c("Default LOM",
                "CLOM(X5X6)", "CLOM(X7X8)", "CLOM(X5678)",
                "MSLOM(X7X8)", "MSLOM(X1278)", "MSLOM(X3478)",
                "MSLOM(X5678)", "MSLOM(X123478)", "MSLOM(all)",
                "DR"),
  ...
  model_betas = list(
    ...
    `CLOM(X7X8)`  = extract_betas(fit_clom_X7_X8),
    `CLOM(X5678)`   = extract_betas(fit_clom_X5678),
    `MSLOM(X7X8)` = extract_betas(fit_msl_X7_X8),
    `MSLOM(X1278)`  = extract_betas(fit_msl_X1278),
    `MSLOM(X3478)`  = extract_betas(fit_msl_X3478),
    `MSLOM(X5678)`  = extract_betas(fit_msl_X5678),
    `MSLOM(X123478)`  = extract_betas(fit_msl_X123478),
    `MSLOM(all)`  = extract_betas(fit_msl_all),
