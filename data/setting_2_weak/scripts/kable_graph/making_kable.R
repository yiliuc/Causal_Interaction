...
make_reri_table <- function(summary_table, beta0) {
  ...
  conditional_reri_or <- exp(0.3 + 0.4 + 0.8) - exp(0.3) - exp(0.4) + 1
  ...
  summary_table <- summary_table %>% 
    mutate(
      ...
      bias   = case_when(
        Model %in% c("CLOM(X5X6)", "CLOM(X7X8)", "CLOM(X5678)", "DR")   ~ abs(est_b3 - conditional_reri_or),
        TRUE                                           ~ abs(est_b3 - truth_or)
      ),
      bias_per   = case_when(
        Model %in% c("CLOM(X5X6)", "CLOM(X7X8)", "CLOM(X5678)", "DR")   ~ round((abs(est_b3 - conditional_reri_or))/conditional_reri_or * 100, 2),
